...
{
	program->sprite.reference_bgd = mlx_xpm_file_to_image(program->mlx,
			"./textures/Background.xpm", &program->isize[0],
			&program->isize[1]);
	...
	program->sprite.reference_player = mlx_xpm_file_to_image(program->mlx,
			"./textures/player_01.xpm", &program->isize[0], &program->isize[1]);
}
...
void	ft_init_window(t_program *program, t_map *map)
{
	...
	{
		ft_printf("Map size is exceeding screen size!\n");
		free_table(map->tab);
		ft_error();
	}
	program->mlx = mlx_init();
	program->win = mlx_new_window(program->mlx, program->lenght,
			program->height, "Hello so_long!");
